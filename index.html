...
<body>
  ...
  <!-- Nueva pestaña en la calculadora -->
<button class="tab-button" onclick="switchTab('corriente')">Cálculo de Corriente</button>

<!-- Contenido de la nueva pestaña -->
<div id="corriente" class="tab-content">
  <h2>Cálculo de Corriente (Monofásico)</h2>
  <p><strong>Fórmulas:</strong></p>
  <ul>
    <li>\[ I = \frac{P}{V} \quad \text{(fórmula simplificada)} \]</li>
    <li>\[ I = \frac{P}{V \cdot \cos(\varphi)} \quad \text{(fórmula completa)} \]</li>
  </ul>
  <label>Potencia \(P\) (kW): <input type="number" id="potenciaKw" /></label><br>
  <label>Tensión \(V\) (V): <input type="number" id="voltaje" /></label><br>
  <label>Cos(\(\varphi\)) (opcional): <input type="number" step="0.01" id="cosphiCorriente" /></label><br>
  <button onclick="calcCorriente()">Calcular</button>
  <div id="corriente-output"></div>
</div>
  ...
  <script>
    ...
    function calcSeccionConductor() {
      ...
      const formula = `SC = \\frac{2 \\cdot ${p.toFixed(4)} \\cdot ${l.toFixed(4)} \\cdot ${i.toFixed(4)} \\cdot ${cosphi.toFixed(4)}}{${vp.toFixed(4)}} = ${sc.toFixed(4)}\\ \\text{mm}^2`;
    ...
    function calcSeccionTrifasico() {
      ...
      const formula = `SC = \\frac{\\sqrt{3} \\cdot ${p.toFixed(4)} \\cdot ${l.toFixed(4)} \\cdot ${i.toFixed(4)} \\cdot ${cosphi.toFixed(4)}}{${vp.toFixed(4)}} = ${sc.toFixed(4)}\\ \\text{mm}^2`;
    ...
    function calcCaida() {
      ...
      const vp = (2 * p * l * i) / sc;
      const formula = `VP = \\frac{2 \\cdot ${p.toFixed(4)} \\cdot ${l.toFixed(4)} \\cdot ${i.toFixed(4)}}{${sc.toFixed(4)}} = ${vp.toFixed(4)}\\ V`;
    ...
    function calcLongitud() {
      ...
      const l = (sc * vp) / (2 * p * i);
      const formula = `L = \\frac{${sc.toFixed(4)} \\cdot ${vp.toFixed(4)}}{2 \\cdot ${p.toFixed(4)} \\cdot ${i.toFixed(4)}} = ${l.toFixed(4)}\\ \\text{m}`;
